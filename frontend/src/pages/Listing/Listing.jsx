...
export default function Listing({ type, saved, onSave }) {
 ...
 let results=stories.filter(story=> type==='saved'?saved.includes(story.id):type==='search'?`${story.title} ${story.description} ${story.category} ${story.author}`.toLowerCase().includes(query.toLowerCase()):type==='trending'?true:story.category===categoryName)
