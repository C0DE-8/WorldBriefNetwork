  ...
  return <article className={s.comment}>
    ...
    <div className={s.commentContent}>
      ...
      <div className={s.commentActions}>
        <button className={liked ? s.liked : ''} onClick={() => onLike(comment.id)}><Icon name="heart" size={14} fill={liked ? 'currentColor' : 'none'} /> {comment.likes}</button>
...
export default function Conversation({ storyId }) {
  ...
  useEffect(() => { api(`/stories/${storyId}/comments?sort=${sort}`).then(({ data }) => setComments(data)).catch(() => {}) }, [storyId, sort])

  async function post(event) {
    event.preventDefault()
    if (!message.trim() || !user) return
    const { data: comment } = await api(`/stories/${storyId}/comments`, { method: 'POST', body: JSON.stringify({ text: message.trim() }) })
  ...
  async function reply(commentId, text) {
    const { data } = await api(`/comments/${commentId}/replies`, { method: 'POST', body: JSON.stringify({ text }) })
    setComments(comments.map((comment) => comment.id === commentId ? { ...comment, replies: [...(comment.replies || []), data] } : comment))
  }
  ...
  async function like(commentId) {
    ...
    await api(`/comments/${commentId}/reaction`, { method: liked ? 'DELETE' : 'PUT' })
    setComments((current) => current.map((item) => item.id === commentId ? { ...item, liked: !liked, likes: item.likes + (liked ? -1 : 1) } : item))
  }
  ...
  return <section className={s.conversation} aria-labelledby="conversation-heading">
    ...
    <div className={s.comments}>{visibleComments.map((comment) => <Comment key={comment.id} comment={comment} user={user} onReply={reply} onLike={like} liked={comment.liked} replying={replying} setReplying={setReplying} />)}</div>
    <div className={s.guidelines}><Icon name="shield" size={15} /><span><b>A better conversation.</b> Challenge ideas, not people.</span><Link to="/about">Community guide <Icon name="arrow" size={12} /></Link></div>
    </div>}
  </section>
}
